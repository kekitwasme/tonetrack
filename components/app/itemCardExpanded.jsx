...
export default ({ selectedItem, updating }) => {
  ...
  const addItem = (updatedItem, of) => {
    userData[of].items = userData[of].items.map((item) =>
  ...
  const getRoutines = () => {
    const selectRoutine = (id) =>
      setCurrentItem({ routineIDs: [...currentItem.routineIDs, id] })

    const deselectRoutine = (id) =>
      setCurrentItem({
        routineIDs: currentItem.routineIDs.filter(
    ...
    return (
      <>
        {userData.routines.items.map(({ id, name }) => {
          ...
          return selected ? (
            ...
            >
              <span className="text-zinc-900 w-fit whitespace-nowrap">
                {name}
              </span>
              <Image src={deleteRoutineIcon} alt="delete routine icon" />
            </div>
          ) : (
            <div
              key={id}
            ...
            >
              <span className="text-white  h-fit w-fit whitespace-nowrap">
                {name}
              </span>
            </div>
          )
  ...
  const getTasks = () => {
    const selectTask = (id) =>
      setCurrentItem({ taskIDs: [...currentItem.taskIDs, id] })
    ...
    return userData.tasks.items.map(({ id, name }) => {
      ...
      return selected ? (
        <div>{/* styling when routine is selected */}</div>
      ) : (
        <div onClick={() => selectTask(id)}>
          {/* styling when not selected */}
        </div>
      )
    })
  }
  ...
  const getLayout = () => {
    return (
      <div
        className="inset-0 flex justify-center fixed z-10 bg-opacity-0 lg:backdrop-blur-md lg:bg-indigo-950 lg:bg-opacity-90 "
        onClick={exit}
      >
        <div
          style={{
            ...
            <>
              <div className="flex flex-col space-y-7 w-4/5 mt-20">
                <div className="flex flex-col space-y-2">
                  ...
                  <input
                    className="text-lg text-black focus:outline-none h-10"
                    name="name"
                    id="name"
                  ...
                  >
                    ...
                    <div className="flex flex-col">
                      ...
                      <div className="flex col-auto gap-y-5 gap-x-3 justify-start items-start h-fit flex-wrap">
                        <div className="flex flex-row item-center justify-center bg-violet-100 h-10 w-fit px-2 py-2 gap-x-1 text-base font-semibold rounded-[10px] ">
                          <span className="text-zinc-900 w-fit whitespace-nowrap">
                            Lower Body
                          </span>
                          <Image
                            src={deleteRoutineIcon}
                            alt="delete routine icon"
                          />
                        </div>
                        <div className="flex flex-row item-center justify-center bg-violet-100 h-10 w-fit px-2 py-2 gap-x-1 text-base font-semibold rounded-[10px] ">
                          <span className="text-zinc-900 w-fit whitespace-nowrap">
                        ...
                        <div className="item-center border-2 h-10 w-fit px-2 py-1.5 text-base font-semibold rounded-[10px] ">
                          <span className="text-white  h-fit w-fit whitespace-nowrap">
                            Cool-down
                          </span>
                        </div>
